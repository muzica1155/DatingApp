...
namespace API.Controllers
{
    ...
    public class MessagesController : BaseApiController //drive from this 
    {
       ...
       private readonly IUnitOfWork _unitOfWork;
        public MessagesController( IUnitOfWork unitOfWork, IMapper mapper) //changes the unit of work
        ...
        //changes of unit of work
        {
            _mapper = mapper;
            _unitOfWork = unitOfWork;

            //changes unit of work
            // _messageRepository = messageRepository;
            // _userRepository = userRepository;
            //changes unitofwork
        }
  //huge changes after SignalR & unit of work
...
//            if (await _unitOfWork.Complete()) return Ok(_mapper.Map<MessageDto>(message));//change unit of work 
...
//huge changes after SignalR & unit of work
        ...
        //bcoz we r adding the pages onto our header 
        {
           ...
           var messages = await _unitOfWork.MessageRepository.GetMessagesForUser(messageParams);//change unit of work
            // var messages = await _messageRepository.GetMessagesForUser(messageParams);//change unit of work
       ...
       //change unit of work
        ...
        //    return Ok(await _messageRepository.GetMessageThread(currentUsername, username)); //.GetMessageThread(currentUsername, username)
        //    //the other username which means username in this case 
        //  } // not using this anymore after unit of work
        ...
        public async Task<ActionResult> DeleteMessage(int id)//we don't return anything from deletions 
        {
            var username = User.GetUsername();// we gonna need to get a hold of our user 1st we'll save our username
             ...
             var message = await _unitOfWork.MessageRepository.GetMessage(id);//change unit of work
...
// go to the messagerepository
        ...
        if (message.SenderDeleted && message.RecipientDeleted) 
                ...
                _unitOfWork.MessageRepository.DeleteMessage(message);//change unit of work
  ...
  //it from the server  we;ll get the message reppsitory// after that we can do the 
    ...
    // if (await _messageRepository.SaveAllAsync()) return Ok();//change unit of work
