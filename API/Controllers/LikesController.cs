...
namespace API.Controllers
{
    ...
    public class LikesController : BaseApiController// derive from our base API controller
    {
        ...
        // we r gooing to need to bot the user repository & like the repository
        private readonly IUnitOfWork _unitOfWork;

        public LikesController(IUnitOfWork unitOfWork)//changes unitofwrok
        {
                                                                ...
                                                                //in this case 
        {
            ...
            // var likedUser = await _userRepository.GetUserByUsernameAsync(username);//changes unitofwork
            // we need ot get hold of the user that we r liking 
            
            var likedUser = await _unitOfWork.UserRepository.GetUserByUsernameAsync(username);//changes unitofwork
            var sourceUser = await _unitOfWork.LikesRepository.GetUserWithLikes(sourceUserId);//changes unitofwork
            ...
            var userLike = await _unitOfWork.LikesRepository.GetUserLike(sourceUserId, likedUser.Id); //cahnges units of work
        ...
        {// 
         ...
         //action result doesn't work so well with an interface like enumerable So waht we do Var user
            ...
            var users = await _unitOfWork.LikesRepository.GetUserLikes(likesParams);//changes units of work
